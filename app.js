...
(function(){
  ...
  function buildSide(side, dest, isAttack){
    ...
    if(isAttack){ moveLabel('moveSelect', dest); relabel('moveSelect','技'); var moveLabelEl=labelOf('moveSelect'); if(moveLabelEl){ moveLabelEl.classList.remove('v101i-main-label'); moveLabelEl.classList.add('v101j2-label-only'); } var moveZone=make('div','v082h-zone'); moveZone.id='v082hMoveZone'; dest.appendChild(moveZone); moveLabel('critical', dest); }
  }
  ...
  function addAbilityPanel(side, dest){
    ...
    var abilitySel=q(side+'AbilitySelect');
    if(abilitySel){
      var oldAbilityLabel=abilitySel.closest('label');
      var newAbilityLabel=document.createElement('label');
      newAbilityLabel.appendChild(document.createTextNode('特性選択'));
      newAbilityLabel.appendChild(abilitySel);
      wrap.appendChild(newAbilityLabel);
      if(oldAbilityLabel && oldAbilityLabel.parentNode) oldAbilityLabel.parentNode.removeChild(oldAbilityLabel);
    }
  ...
  function currentPokemon(side){
    ...
    return (D.pokemons||[]).find(function(p){ return p.id===id; });
  }
  ...
  function buildConditionDetails(title, ids, doubleIds){
    ...
    if(!section) return null;
    var d=details(title,'sub-card v082h-section-details',false);
  ...
  function currentLayoutZonesBuilderV2(){
    safeStep('installLayout', installLayout);
    safeStep('setupZones', setupZones);
    safeStep('restructureConditions', restructureConditions);
    document.body.classList.add('v109j-layout-zones-builder-v2-owner');
    var markIds = [
      'v082hBasicGrid','v082hToolbar','v082hMoveZone','v082hMoveDetails','v082hAbilityDetails','v082hItemDetails','v082hTeraDetails','v082hDefenderAbilityDetails','v082hResultPanel'
    ];
    markIds.forEach(function(id){ var el=q(id); if(el){ el.classList.add('v109j-layout-zones-builder-v2-owned','v109c-owned-layout-zone'); } });
  }
  function shadowInitLayoutZones(){
    currentLayoutZonesBuilderV2();
    document.body.classList.add('v109q-shadow-init-layout-zones-owner');
  }
  function init(){ document.body.classList.add('v082h-ui'); shadowInitLayoutZones(); bind(); refreshAll(); setupResult(); }
...
// v0.94 single-entry init orchestrator (Stage 1 of unification)
