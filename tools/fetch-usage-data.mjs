...
const KNOWN_CHAMPIONS_ONLY_SLUGS = {
  ...
  // 以下、実際のGitHub Actions実行結果(診断ログで出力した「今日のpokemon配列内の実エントリ」)
  // で判明した、本物のshowdownId。以前 pokemonPages(サイトの静的ページ一覧)から推測した
  // 値は、実際のpokemon配列(=対戦データが存在するエントリのみを含む、より狭い集合)の
  // 値と食い違っており、全滅していた。今回は実際にデータが存在することを確認済みの
  // showdownIdを直接指定する。
  //
  // タウロス(パルデア地方3品種): それぞれ独立したランキングが実在する。
  'ケンタロス(コンバット種)': 'taurospaldeacombat',
  'ケンタロス(ブレイズ種)': 'taurospaldeablaze',
  'ケンタロス(ウォーター種)': 'taurospaldeaaqua',
  // ビビヨン: 実際には「Vivillon-Fancy」という単一エントリのみ存在する。
  'ビビヨン': 'vivillonfancy',
  // ニャオニクス(メオスティック): 性別で独立したエントリが実在する。オスは基本種のIDで
  // 橋渡しできるため、メスのみここで明示指定する。
  'ニャオニクス(メス)': 'meowsticf',
  // ギルガルド: 独立したエントリはなく、実際の橋渡し先は単に「aegislash」。
  'ギルガルド(ブレードフォルム)': 'aegislash',
  // イエッサン(インディーデ): 性別で独立したエントリが実在する。オスは基本種のIDで橋渡し
  // できるため、メスのみここで明示指定する。
  'イエッサン(メス)': 'indeedeef',
  // モルペコ: 「はらぺこもよう」を区別する独立エントリはなく、単一の「morpeko」に統合。
  'モルペコ(はらぺこもよう)': 'morpeko',
  // イダイトウ(バスカレジ): 性別で独立したエントリが実在する。オスは基本種のIDで橋渡し
  // できるため、メスのみここで明示指定する。
  'イダイトウ(メス)': 'basculegionf',
  // イッカネズミ(マウストドン): 当初「単一ページに統合」と想定していたが、実際には
  // 「maushold」と「mausholdfour」という2つの独立したエントリが実在することが判明した。
  // 家族の人数ごとに、それぞれ対応する実エントリへ個別に紐づける。
  // イッカネズミ(マウストドン): 'maushold'エントリはロースターには存在するものの、実際の
  // 対戦データ(行)を持たない可能性が高く反映されなかった。動作確認済みの'mausholdfour'に
  // 両方とも統一する(家族の人数による競技的な違いはないため、同一データで問題ない)。
  'イッカネズミ(3びきかぞく)': 'mausholdfour',
  'イッカネズミ(4ひきかぞく)': 'mausholdfour',
  // イキリンコ(スカウビリー): 実際には「squawkabilly」(基本種)と「squawkabillyyellow」の
  // 2エントリのみ実在する。対戦での機能ペア(グリーン/ブルーが同一グループ、イエロー/
  // ホワイトが同一グループ)に合わせて、それぞれの代表エントリへ統合する。
  'イキリンコ(グリーンフェザー)': 'squawkabilly',
  'イキリンコ(ブルーフェザー)': 'squawkabilly',
  'イキリンコ(イエローフェザー)': 'squawkabillyyellow',
  'イキリンコ(ホワイトフェザー)': 'squawkabillyyellow',
  // イルカマン(パルデア): フォルム別の独立エントリはなく、単一の「palafin」に統合。
  'イルカマン(マイティ)': 'palafin',
};
